...
/config/local/saluteora/database/content/pages/home.json

{
    "type": "landing-page",
    "data": {
        "view": "pub_theme::components.blocks.hero.landing-page",
        "title": "Salute Orale per Gestanti",
...
3. **JSON Loading** → `/config/local/saluteora/database/content/pages/home.json`
...
test('homepage renders with hero block', function () {
    ...
    $response->assertOk()
        ->assertSee('Salute Orale per Gestanti')
